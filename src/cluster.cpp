...
void proximity2D(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol,
				 std::vector<bool> &visited_set, std::vector<int> &cluster, int i)
{

	cluster.push_back(i);
	visited_set[i] = true;
	std::vector<int> nearby_points = tree->search(points[i], distanceTol);
	for(int it = 0; it < nearby_points.size(); it++)
	{
		if(!visited_set[nearby_points[it]])
		{
			proximity2D(points, tree, distanceTol, visited_set, cluster, nearby_points[it]);
		}
	}
}

std::vector<std::vector<int>> euclideanCluster2D(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{

	// TODO: Fill out this function to return list of indices for each cluster

	std::vector<std::vector<int>> clusters; //Lis tof clusters
	std::vector<bool> visited_set(points.size(), false);
	for (int i_p = 0; i_p < points.size(); i_p++)
	{
		//tree->search(points[i_p], distanceTol);
		//Check if this point has not been visited
		//std::unordered_set<int>::iterator it = visited_set.find(i_p);
		if (!visited_set[i_p])
		{
			std::cout<<"Coming here "<<i_p<<std::endl;
			std::vector<int> cluster;
			//this is not visited so visit now
			proximity2D(points, tree, distanceTol, visited_set, cluster, i_p);
...
// 	//
// 	std::vector<std::vector<int>> clusters = euclideanCluster2D(points, tree, 3.0);
// 	//
